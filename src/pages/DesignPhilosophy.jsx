...
function Section({ eyebrow, title, children, divider = true }) {
  return (
    <section className={`py-12 sm:py-16 text-center ${divider ? "border-t border-band/30" : ""}`}>
      <div className="flex justify-center">
        <Eyebrow>{eyebrow}</Eyebrow>
      </div>
      <h2 className="text-[24px] sm:text-[28px] font-semibold text-heading mb-5">{title}</h2>
      <div className="space-y-4 text-[15px] sm:text-[16px] leading-relaxed text-body max-w-2xl mx-auto">
        {children}
      </div>
    </section>
  );
...
const THREAD_ITEMS = [
  "In IoT, that was a real-time command center reading 2,300+ live camera feeds so operators could act on a state instead of guessing at one.",
  <>In benefits, that was turning IRS compliance rules and life-event workflows into signals people could read <em>before</em> they made a costly mistake.</>,
  "In design systems, that was unifying two disconnected systems into one framework so teams stopped guessing which pattern was \"correct\" this week.",
];

const PROCESS_ITEMS = [
  <><strong className="font-semibold text-heading">I lead with research, not opinion.</strong> Every major redesign I've shipped — HSA, enrollment, the chatbot — started from usability testing and real friction data, not a hunch. I've done this at enterprise scale and at the scale of one sheepherder's actual day — the method doesn't change, just the room.</>,
  <><strong className="font-semibold text-heading">I build the system, not just the screen.</strong> Two of the biggest wins in my career are design <em>systems</em> work — unifying frameworks so decisions get faster for everyone, not just prettier for one team.</>,
  <><strong className="font-semibold text-heading">I know what the code costs.</strong> Starting in web development means I don't hand off specs that quietly become someone else's problem in sprint planning.</>,
  <><strong className="font-semibold text-heading">I measure what I ship.</strong> Task time, completion rate, CSAT, ticket volume — if I can't point to a number, I don't consider the redesign done.</>,
];
...
const COLLABORATION_ITEMS = [
  "Bring me the research, not just the ticket. I'll move faster with the \"why\" than with a polished mock to approve.",
  "Loop me in before the architecture is locked. I can save a rebuild if I see it early.",
  "Tell me the real constraint, not the diplomatic version of it. I'd rather design inside a hard limit than discover it in QA.",
];
      ...
      <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-32">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <div className="pt-20 sm:pt-24 pb-4">
            ...
            <p className="text-[16px] sm:text-[18px] italic text-body leading-relaxed max-w-xl mx-auto">
          ...
          <Section eyebrow="Origin" title="Where this actually began">
            ...
            <Statement>From there the path wasn't linear, it was escalating:</Statement>
            <div className="font-mono text-[14px] text-heading bg-card border border-band/40 rounded-xl px-5 py-4 max-w-md mx-auto">
          ...
          <Section eyebrow="Philosophy" title="The intersection I keep coming back to">
            ...
            <h3 className="text-[12px] font-semibold uppercase tracking-[0.06em] text-muted mt-8 mb-2">
              Three rules I don't break
            </h3>
            <div>
              {LAWS.map((law, i) => (
                <div key={law.title} className={`py-6 ${i > 0 ? "border-t border-band/30" : ""}`}>
                  <span className={`inline-block w-3.5 h-3.5 rounded-full mb-3 ${law.dotClass}`} />
          ...
          <Section eyebrow="Pattern" title="The thread running through all of it">
            ...
            {THREAD_ITEMS.map((item, i) => (
              <Statement key={i}>{item}</Statement>
            ))}
            <Statement>
          ...
          {/* PERSONAL */}
          <Section eyebrow="Personal" title="Outside of work">
            <Statement>
          ...
          <Section eyebrow="Beliefs" title="A few things I hold as true">
            {BELIEFS.map((belief) => (
              <Statement key={belief}>{belief}</Statement>
            ))}
          </Section>

          {/* COLLABORATION */}
          <Section eyebrow="Collaboration" title="If we end up working together">
            {COLLABORATION_ITEMS.map((item, i) => (
              <Statement key={i}>{item}</Statement>
            ))}
          </Section>
          ...
          {/* CLOSING */}
          <div className="py-16 sm:py-20 border-t border-band/30">
            <TrafficLight size="sm" />
